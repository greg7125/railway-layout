...
void loop() {
  ...
  if (serialBite = 1 or millis() - myTimer1 >= 5000) {   // таймер на 5с
    ...
    tmr2.start(2000);
    if (tmr2.ready())
    { digitalWrite(PIN_RELAY, LOW); digitalWrite(PIN_2RELAY, LOW); //1-2 выгрузка
    }
  ...
  //void yield() {
  // а тут можно прикрутить что то что будет работать в многопотоке даже если будет delay задержка, но я вроде всю эту гадость выкнул
  //}
